...
// The DestiniesFunc type is an adapter to allow the use of ordinary
// function as Destinies mutator.
type DestiniesFunc func(context.Context, *ent.DestiniesMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DestiniesFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DestiniesMutation); ok {
		return f(ctx, mv)
	}
...
// Mutate calls f(ctx, m).
func (f TestimoniesFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TestimoniesMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TestimoniesMutation", m)
}
